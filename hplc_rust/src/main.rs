...
/*
fn reader_from_path(path: &Path) -> (BufReader<File>) {
    let reader = BufReader::new(File::open(path).unwrap());
    reader;
}
*/

fn extract_data_from_file(path: &Path) -> (Option<String>, HashMap<String,String>) {
        ...
        if process_indicator {
            ...
            if v.len() ==  6 {
             // data_map.insert(v[1].to_string(), v[5].to_string());
                data_map.insert(v[1].to_owned(), v[5].to_owned());
            }
        }   
    }   
